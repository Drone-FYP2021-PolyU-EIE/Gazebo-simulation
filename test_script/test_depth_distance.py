...
class test_depth_distance():
    ...
    def depth_cb(self, msg):
        ...
        print ('center depth: {}'.format(self.depth_array[self.center_idx[0], self.center_idx[1]]))
        self.height = self.depth_array.shape[0]
        self.width = self.depth_array.shape[1]


        # calculate the box XY
        self.depth_image_centre = [(self.height/2)+desired_box_y_offset, (self.width/2)+desired_box_x_offset]
        ...
        self.lefttop_corner_distance = self.depth_array[self.x1, self.y1]
        self.righttop_corner_distance = self.depth_array[self.x1, self.y2]
        self.leftdown_corner_distance = self.depth_array[self.x2, self.y1]
        self.rightdown_corner_distance = self.depth_array[self.x2, self.y2]
